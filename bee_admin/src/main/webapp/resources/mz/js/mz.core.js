...
var MZ = {
     ...
     * @returns {string} mz.index.js中有引用
     */
    ...
    /**
     *  * options:{idKey: 'id', name: 'name', pIdKey: 'parentId'};
     * data;List<MenuItemConfEntity>  :id,name,parentId ^^
     * jztree:两种格式：标准的json数据；
     * 例如：
     var nodes = [
     {name: "父节点1", children: [
     {name: "子节点1"},
    ...
    parseSimpleData: function (options, data) {
        ...
        for (i = 0, l = data.length; i < l; i++) {
            tmpMap[data[i][idKey]] = data[i];//parentId不为空 通过此保存data 散列表
        }
        for (i = 0, l = data.length; i < l; i++) {
            if (tmpMap[data[i][pIdKey]] && data[i][idKey] != data[i][pIdKey]) {// 如果此条记录存在父类
                if (!tmpMap[data[i][pIdKey]][children])tmpMap[data[i][pIdKey]][children] = [];
                data[i][name] = data[i][name];
                tmpMap[data[i][pIdKey]][children].push(data[i]); //存在父类，那么把此条记录同时赋给父类.chileren
...
//            $(that._boxId).remove();
            ...
            if(type == this._types.confirm) {
                this._currentBox = $(that._boxContent).appendTo('body').noty({
                    ...
                    callback:{
                        afterShow: function () {
//                            if(buttons.okClick) that._okClick = buttons.okClick;
...
//                            if(buttons.okClick) that._okClick = buttons.okClick;
         ...
         * 询问提示框（是/否）
         * @param msg 提示消息
         * @param buttons 其它可选项 {okId,okClass,okValue,okClick,cancelId,cancelClass,cancelValue,cancelClick}
         */
...
//            if(this._okClick) this._okClick();
            this._currentBox.close();
            $(this._boxId).remove();
        }
    },
    templates:{
        dataDetailView:'<tr class="row-detail-inserted">' +
            '<td class="td-detail-inserted-left"></td>' +
            '<td class="td-detail-inserted-right" colspan="#colspan#">' +
            '<div id="ddv-#id#" class="tableScroller" style="width: #width#px;border: 1px solid #CCCCCC;height: #height#px">正在加载中...</div></td></tr>'
    }
};
window.MZ = MZ;
...
(function ($) {
    ...
    $.fn.extend({
        /**
         * 封装Ajax提交函数
         * 1.首页href处加载用到 传递参数 url:url data :kong
        ...
        ajaxUrl: function (options) {
            ...
            $.ajax({
                type: options.type || 'POST', //无type类型默认为post
                url: options.url,
                data: options.data,
                cache: false,
                success: function (responseData) {
                    var json = MZ.jsonEval(responseData);//json 前台Ajax对象中有message statusCode等
                    if (json[MZ.keys.successKey] == MZ.successValue.failed) {
                        if (json[MZ.keys.messageKey]) MZ.alertMsg.error(json[MZ.keys.messageKey]);
                    } else {
                        $this.html(responseData).initUI(); //initUI(); 成功时装载html
    ...
    $.extend(String.prototype, {
        ...
        replaceTmById:function(_box){
            ...
            //查看正则API this.replace(正则，函数）函数参数$1-$99代表第一个到地99个正则
            //new RegExp(pattern, attributes); g 全局匹配，i:区分大小写匹配，m:多行匹配
            //目前看到的就是 list 点击新增等按钮时候调用找到table下的input 的value 就是ids

            return this.replace(RegExp("({[A-Za-z_]+[A-Za-z0-9_]*})","g"), function($1){
                //把{something} 替换为id 为$1 即是{something}的value；
                var $input = $parent.find("#"+$1.replace(/[{}]+/g, "")); //去掉空格
